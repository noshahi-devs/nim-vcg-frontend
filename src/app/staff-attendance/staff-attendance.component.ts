...
export class StaffAttendanceComponent implements OnInit {
  ...
  markAllPresent(): void {
    ...
    Swal.fire('All Marked Present', '', 'success');
  }

  markAllAbsent(): void {
    this.staffMembers.forEach(s => s.status = 'Absent');
    Swal.fire('All Marked Absent', '', 'info');
  }
  ...
  const attendancePayload = this.staffMembers.map(s => ({
    attendanceId: 0,
    date: new Date(this.selectedDate),
    type: 1, // staff type
    attendanceIdentificationNumber: s.staffId, // ✅ FIX
    staffId: s.staffId,                 // crucial! this was missing
    isPresent: s.status === 'Present',
    description: s.remarks || ''
  }));

  // Send all in a single loop or batch API if supported
  attendancePayload.forEach(attendance => {
    this.attendanceService.addAttendance(attendance).subscribe({
      next: () => {},
      error: (err) => console.error('Attendance save error', err)
    });
  });
